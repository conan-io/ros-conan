...
PIP_BUILD_TOOLS = (
    ...
    # colcon-core 0.20+ recognises the underscore-form `install_scripts=` key
    # in setup.cfg (PR colcon/colcon-core#720) and explicitly forwards
    # `--install-scripts $base/lib/<pkg>` to setuptools. That redirects the
    # cli-64 entry-point pair generated by `console_scripts` from the venv
    # default `Scripts/` into `lib/<pkg>/<exec>{.exe,-script.py}`, which is
    # where `ros2 run <pkg> <exec>` looks on Windows. With 0.17.x the
    # directive was silently dropped and the .exe stub never reached lib/.
